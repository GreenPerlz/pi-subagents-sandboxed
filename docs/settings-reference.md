...
Guarded paths are `cwd`, `context`, `model`, `fastMode`, `skills`, `output`, `outputMode`, `reads`, `progress`, `outputSchema`, `share`, `worktree`, `maxSubagentDepth`, every `acceptance.<field>`, and every `sandbox.<field>`. A shared override in a multi-agent launch must be allowed by every affected agent; shared `worktree: true` remains deny-by-default for packaged agents. Cwd is the exception to the ordinary permission rule for existing directories: a path equal to or canonically beneath the trusted actual invoking `ctx.cwd` is implicitly allowed, while canonical outside, ancestor, prefix-sibling, or symlink-escaping paths require explicit `cwd` permission from an agent definition visible at that invoking root. Missing, malformed, non-directory, NUL-containing, and inaccessible paths fail closed. Relative cwd values always anchor at actual invoking `ctx.cwd`; a requested shared cwd never redefines that authorization anchor. Explicit cwd paths still pass normal Git, read-only, trust, and sandbox/mount gates. Group cwd is the fallback for omitted task cwd, while an explicit task cwd takes precedence; this applies consistently to single, top-level parallel, sequential, static parallel, and dynamic fanout. Only a dedicated `canOptOutOfWorktree: true` permission, together with the trusted user-global ceiling, may authorize the narrowing `worktree: false` opt-out.
...
### Isolated Git recovery bundles

An isolated run returns one compact bundle reference in foreground details, async result/status, and intercom projections. The bundle contains child-authored commits plus, when the checkout is dirty, internal staged-state and final worktree recovery commits authored by the runtime for packaging only (they are not authored child commits). When staged B differs from worktree C, optional `stagedSnapshot` identifies B and `recovery` remains the final C tip; both are reachable from one bundle. A commit-required writer with no authored commit or with residual staged/worktree changes is reported as `incomplete`; non-commit-required read-only/review dirty recovery is not incomplete solely because it is dirty.

Portable metadata includes the agent identity, termination state, base/head, recovery and optional staged snapshot/tree refs, incomplete flag, authored commit summary (excluding packaging commits), dirty summary, and explicit bundle-file and raw/canonical payload checksum/size scopes. The returned bundle checksum/size covers the exported bundle file; portable metadata's `bundleChecksum`/`bundleSize` (also exposed as `payloadChecksum`/`payloadSize`) cover the raw Git payload generated before metadata embedding; canonical fields cover the deterministic sorted refs/object-ID manifest. Metadata redacts selected local roots (`/tmp`, `/var/tmp`, `/private/tmp`, `/home`, `/Users`, drive-letter, and UNC paths) at filesystem boundaries, while preserving route-like prose such as `/api/v1`. None cover incidental filesystem paths or metadata presentation. Metadata omits temporary and absolute local filesystem paths while preserving ordinary non-path prose in commit summaries. Consumers can verify the Git bundle with ordinary `git bundle verify` and inspect or fetch its refs with ordinary Git commands; do not treat the runtime recovery commit as authored history without review.

Bundles and their output directories are owner-only. They remain available for seven days by default (`cleanupDays: 7`); cleanup is best-effort, runs at extension startup, is gated to at most once per day, and never removes a runtime worktree whose export failed. `artifacts: false` disables ordinary input/output artifacts but does not disable isolated recovery export. If export fails, the original child/execution error remains visible and details include an actionable preserved worktree path.
...
- **`cwd`** — Sets the working directory shared by affected children. Relative values resolve from the trusted actual invoking `ctx.cwd`, never process cwd or another requested shared cwd. Existing same/canonical descendants are implicitly permitted; canonical outside, ancestor, prefix-sibling, or symlink-escaping paths require explicit `cwd` permission from every affected agent definition visible at the invoking root, then still pass normal Git/read-only/trust/sandbox gates. Group cwd is the fallback for omitted task cwd; explicit task cwd takes precedence. Malformed, missing, non-directory, NUL-containing, and inaccessible paths fail closed.
