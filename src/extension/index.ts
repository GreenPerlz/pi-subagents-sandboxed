...
import { registerSubagentsOverlayCommand, registerSubagentsOverlayShortcut } from "../tui/subagents-overlay.ts";
import { registerSubagentsSettingsCommand } from "../tui/subagents-settings-overlay.ts";
...
• Optional context: { context: "fresh" | "fork" } (default: if any requested agent has defaultContext: "fork", the whole invocation uses fork; otherwise "fresh"; inspect agent defaults via { action: "list" }). Explicit agent-specific overrides (cwd, context, model, skill/skills, output, outputMode, reads, progress, outputSchema, share, maxSubagentDepth, acceptance fields, and sandbox fields) are deny-by-default and must be listed by each target agent's canBeChangedByAgent policy. Cwd is implicitly allowed for an existing path equal to or canonically beneath trusted actual invoking ctx.cwd; canonical outside, ancestor, prefix-sibling, or symlink-escaping paths require explicit cwd permission from definitions visible at that invoking root and still pass Git/read-only/trust/sandbox gates. Relative cwd values anchor at actual invoking ctx.cwd, never process cwd or a requested shared cwd. In groups, group cwd is the fallback and explicit task cwd takes precedence. Denied or malformed/missing/non-directory/NUL cwd values fail before any child starts.
...
• { action: "doctor" } - read-only report for runtime paths, discovery, sessions, and intercom`,
	...
	registerSubagentsOverlayShortcut(pi, state, config);
